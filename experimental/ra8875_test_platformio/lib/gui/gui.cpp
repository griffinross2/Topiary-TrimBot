...
Scene::Scene(ColorRGB888 background_color) {
    m_background_color = rgb888_to_rgb332(background_color);
}
...
Rectangle::Rectangle(Scene* parent, int x, int y, int w, int h,
    ...
    m_color = rgb888_to_rgb332(color);
}
...
void Button::redraw() {
    if (m_visible) {
        ...
        lcd_draw_rectangle(x, y, w, h,
                           m_pressed ? LITERAL_RGB888_TO_RGB332(0xBBBBBB)
                                     : LITERAL_RGB888_TO_RGB332(0x888888));
    }
}
