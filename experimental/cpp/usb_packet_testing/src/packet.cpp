...
int packet_check(const uint8_t *buf, int total_length)
{
    ...
    // Calculate CRC
    PacketCRC calculated_crc = crc16_ccitt(buf, sizeof(PacketHeader) + header->length);

    // Get actual received CRC
    PacketCRC received_crc;
    memcpy(&received_crc, buf + sizeof(PacketHeader) + header->length, sizeof(PacketCRC));
...
int packet_send(const uint8_t *data, int data_length, PacketID id)
{
    ...
    cobs_encode_result res = cobs_encode(encoded_buf, sizeof(encoded_buf), buf, packet_len);
    if (res.out_len < 0)
    {
        return -1;
    }
