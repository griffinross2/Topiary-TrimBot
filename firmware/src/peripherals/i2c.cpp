...
const I2cPinAf i2c_pin_af[4][I2C_PIN_AF_COUNT] = {
    {
        // I2C1
...
static Status i2c_setup(const I2cDevice *dev)
{
    ...
    pin_conf.Pin = BOARD_GPIO_PIN(dev->scl);
    HAL_GPIO_Init(BOARD_GPIO_PORT(dev->scl), &pin_conf);
    ...
    pin_conf.Pin = BOARD_GPIO_PIN(dev->sda);
    HAL_GPIO_Init(BOARD_GPIO_PORT(dev->sda), &pin_conf);
