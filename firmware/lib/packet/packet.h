#pragma once

#include <cstdint>

typedef struct {
    uint8_t type : 6;
    uint8_t ack_req : 1;
    uint8_t ack : 1;
} PacketID;

#define PACKET_TYPE_FILE_START {0x00, 1, 0}
#define PACKET_TYPE_FILE_CHUNK {0x01, 0, 0}
#define PACKET_TYPE_FILE_RESEND {0x02, 0, 0}
#define PACKET_TYPE_FILE_END {0x03, 1, 0}
#define PACKET_TYPE_GCODE {0x10, 0, 1}
#define PACKET_TYPE_STATUS {0x20, 0, 0}
#define PACKET_TYPE_START_SCANNING {0x21, 0, 0}
#define PACKET_TYPE_DONE_SCANNING {0x22, 0, 0}
#define PACKET_TYPE_START_TOOLPATHING {0x23, 0, 0}
#define PACKET_TYPE_DONE_TOOLPATHING {0x24, 0, 0}
#define PACKET_TYPE_START_CUTTING {0x25, 0, 0}
#define PACKET_TYPE_DONE_CUTTING {0x26, 0, 0}
#define PACKET_TYPE_PI_BOOTED {0x27, 0, 0}
#define PACKET_TYPE_CROSS_SECTION {0x30, 0, 0}
#define PACKET_TYPE_CREATE_CROSS_SECTIONS {0x31, 0, 0}
#define PACKET_TYPE_GET_CROSS_SECTION {0x32, 0, 0}

static_assert(sizeof(PacketID) == 1, "PacketType should be 1 byte");

typedef struct {
    PacketID id;
    uint8_t length;
} PacketHeader;

static_assert(sizeof(PacketHeader) == 2, "PacketHeader should be 2 bytes");

typedef uint16_t PacketCRC;

#define PACKET_SIZE(data_len) \
    (sizeof(PacketHeader) + data_len + sizeof(PacketCRC))
#define PACKET_HEADER_SIZE sizeof(PacketHeader)
#define MAX_PACKET_DATA_SIZE (254 - sizeof(PacketHeader) - sizeof(PacketCRC))

bool packet_can_send(int data_length);
int packet_fill(uint8_t* buf, int data_length, PacketID id);
int packet_check(const uint8_t* buf, int total_length);
int packet_send(const uint8_t* data, int data_length, PacketID id);