...
#if (HAS_X_AXIS && X_HOME_DIR) || (HAS_Y_AXIS && Y_HOME_DIR) || \
    (HAS_Z_AXIS && Z_HOME_DIR) || (HAS_I_AXIS && I_HOME_DIR) || \
    (HAS_J_AXIS && J_HOME_DIR) || (HAS_K_AXIS && K_HOME_DIR) || \
    (HAS_U_AXIS && U_HOME_DIR) || (HAS_V_AXIS && V_HOME_DIR) || \
    (HAS_W_AXIS && W_HOME_DIR)
...
// No inactive extruders with SWITCHING_NOZZLE or Průša MMU1 or just 1 E stepper
// exists
...
// Switching extruder has its own servo?
    ...
    (!HAS_SWITCHING_NOZZLE ||      \
     SWITCHING_EXTRUDER_SERVO_NR != SWITCHING_NOZZLE_SERVO_NR)
...
#endif
#if ANY(HAS_Z_SERVO_PROBE, HAS_MAG_MOUNTED_SERVO_PROBE, SWITCHING_EXTRUDER, \
        SWITCHING_NOZZLE)
 ...
 */
#if ANY(TOUCH_MI_PROBE, Z_PROBE_ALLEN_KEY, HAS_Z_SERVO_PROBE, SOLENOID_PROBE, \
        Z_PROBE_SLED, RACK_AND_PINION_PROBE, SENSORLESS_PROBING, MAGLEV4,     \
        MAG_MOUNTED_PROBE, BIQU_MICROPROBE_V1, BIQU_MICROPROBE_V2)
...
#define HAS_REAL_BED_PROBE 1
    ...
    NONE(Z_MIN_PROBE_USES_Z_MIN_ENDSTOP_PIN, Z_SPI_SENSORLESS)
...
#undef XY_PROBE_FEEDRATE
      ...
      ALL(AUTO_BED_LEVELING_UBL, HAS_MARLINUI_MENU))
 ...
 */
#if PROBE_SELECTED || \
    (HAS_MARLINUI_MENU && ANY(DELTA_CALIBRATION_MENU, DELTA_AUTO_CALIBRATION))
...
#define GRID_LOOP(A, B)                             \
    for (uint8_t A = 0; A < GRID_MAX_POINTS_X; ++A) \
        for (uint8_t B = 0; B < GRID_MAX_POINTS_Y; ++B)
#endif
...
#elif ANY(DELTA, POLARGRAPH, POLAR, TRIMBOT)
#define IS_KINEMATIC 1
#else
