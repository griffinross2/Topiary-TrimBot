 ...
 * Simple  : E steps = X + E   ; X steps = X  (E drives a loop, X stays the
 * same) Balanced: E steps = X + E/2 ; X steps = X - E/2  (Dual loop system)
 */
...
// https://reprap.org/wiki/PID_Tuning #define MPCTEMP         // See
...
// (°C) Reference temperature for MPC_HEATER_POWER and MPC_HEATER_ALPHA.
#endif
...
#define MPC_AMBIENT_XFER_COEFF_FAN255 \
    {0.097f}  // (W/K) Heat transfer coefficients from heat block to room air
              // with fan on full.
#endif
...
// extruders #define THERMAL_PROTECTION_BED     // Enable thermal protection for
// the heated bed #define THERMAL_PROTECTION_CHAMBER // Enable thermal
// protection for the heated chamber #define THERMAL_PROTECTION_COOLER  //
// Enable thermal protection for the laser cooling
...
#define TPARA_ARM_X_HOME_POS \
    28.75  // (mm) Measured from shoulder axis to tool holder axis in home
           // position
#define TPARA_ARM_Y_HOME_POS 0  // (mm)
#define TPARA_ARM_Z_HOME_POS \
    250.00  // (mm) Measured from tool holder axis to the floor

// TPARA Workspace offset relative to the tower (position of workspace origin
// relative to robot Tower origin ) This needs to be reasonably accurate as it
// defines the printbed position in the TPARA space.
#define TPARA_OFFSET_X \
    127.0  // (mm) to coincide with minimum radius MIDDLE_DEAD_ZONE_R, and
           // W(0,0,0) is reachable
#define TPARA_OFFSET_Y 0.0  // (mm)
#define TPARA_OFFSET_Z 0.0  // (mm)

// TPARA tool connection point offset, relative to the tool moving frame origin
// which is in the last linkage axis, (TCP: tool center/connection point) of the
// robot, the plane of measured offset must be alligned with home position plane
...
#define TPARA_TCP_OFFSET_Z \
    -65.0  // (mm) Tool flange (bottom): -6 (caution as Z 0 posiion will crash
           // second linkage to the floor, -35 is safe for testing with no
           // tool), extruder tool (depends on extruder): -65.0

#define FEEDRATE_SCALING  // Convert XY feedrate from mm/s to degrees/s on the
                          // fly

// Radius around the center where the arm cannot reach
// For now use a hardcoded uniform limit, although it should be calculated, or
// fix a limit for each axis angle
...
// #define S_CURVE_FACTOR 0.25    // Initial and final acceleration factor,
// ideally 0.1 to 0.4.
...
#define NUM_MOTION_SENSORS \
    1  // Number of sensors, up to one per extruder. Define a FIL_MOTION#_PIN
       // for each.
...
#define HOMING_FEEDRATE_MM_M {0, (20 * 60), (20 * 60), (15 * 60), (15 * 60)}
...
// #define USE_STRING_HEADINGS       // Use string headings for Creality UI
// instead of images #define USE_STRING_TITLES         // Use string titles for
// Creality UI instead of images
#endif
