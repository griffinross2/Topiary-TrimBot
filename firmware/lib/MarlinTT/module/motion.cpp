...
#if HAS_Z_AXIS
                     // If Z has been powered on trust that the real Z is >=
                     // current_position.z
...
static_assert(ABS(X_HOME_POS - (TPARA_ARM_X_HOME_POS + TPARA_TCP_OFFSET_X -
                                TPARA_OFFSET_X)) < 0.01f,
              "X_HOME_POS should be equal to (TPARA_ARM_X_HOME_POS + "
              "TPARA_TCP_OFFSET_X - TPARA_OFFSET_X)");
static_assert(ABS(Y_HOME_POS - (TPARA_ARM_Y_HOME_POS + TPARA_TCP_OFFSET_Y -
                                TPARA_OFFSET_Y)) < 0.01f,
              "Y_HOME_POS should be equal to (TPARA_ARM_Y_HOME_POS + "
              "TPARA_TCP_OFFSET_Y - TPARA_OFFSET_Y).");
static_assert(ABS(Z_HOME_POS - (TPARA_ARM_Z_HOME_POS + TPARA_TCP_OFFSET_Z -
                                TPARA_OFFSET_Z)) < 0.01f,
              "Z_HOME_POS should be equal to (TPARA_ARM_Z_HOME_POS + "
              "TPARA_TCP_OFFSET_Z - TPARA_OFFSET_Z).");
#endif
 ...
 */
xyze_pos_t current_position = LOGICAL_AXIS_ARRAY(
    0, X_HOME_POS, Y_HOME_POS,
#ifdef Z_IDLE_HEIGHT
    Z_IDLE_HEIGHT
#else
    Z_HOME_POS
...
void reset_hotend_offsets() {
    constexpr float tmp[3][HOTENDS] = {HOTEND_OFFSET_X, HOTEND_OFFSET_Y,
                                       HOTEND_OFFSET_Z};
    static_assert(!tmp[X_AXIS][0] && !tmp[Y_AXIS][0] && !tmp[Z_AXIS][0],
...
void report_real_position() {
    ...
    xyze_pos_t npos = LOGICAL_AXIS_ARRAY(
        planner.get_axis_position_mm(E_AXIS), cartes.x, cartes.y, cartes.z,
        cartes.i, cartes.j, cartes.k, cartes.u, cartes.v, cartes.w);
...
// Report the logical current position according to the most recent G-code
// command
...
/**
 * Report the logical current position according to the most recent G-code
 * command. The planner.position always corresponds to the last G-code too. This
 * makes M114 suitable for debugging kinematics and leveling while avoiding
 * planner sync that definitively interrupts the printing flow.
 */
...
#define HOMING_CURRENT(A) \
    TERN(EDITABLE_HOMING_CURRENT, homing_current_mA.A, A##_CURRENT_HOME)

// Saves the running current of the motor at the moment the function is called
// and sets current to CURRENT_HOME
...
#define _MAP_SAVE_SET(A) OPTCODE(A##_HAS_HOME_CURRENT, _SAVE_SET_CURRENT(A))
    ...
    if (DEBUGGING(LEVELING))
        DEBUG_ECHOLNPGM("Setting homing driver current");
    ...
    switch (axis) {
        ...
        case X_AXIS:
        case Y_AXIS:
            MAP(_MAP_SAVE_SET, X, X2, Y, Y2);
            break;
        case Z_AXIS:
            MAP(_MAP_SAVE_SET, Z, Z2, Z3, Z4);
            break;
    }
    ...
    switch (axis) {
        ...
        case X_AXIS:
        case Z_AXIS:
            MAP(_MAP_SAVE_SET, X, Z);
            break;
        case Y_AXIS:
            MAP(_MAP_SAVE_SET, Y, Y2);
            break;
    }
    ...
    switch (axis) {
        default:
            break;
        case X_AXIS:
            MAP(_MAP_SAVE_SET, X, X2);
            break;
        case Y_AXIS:
        case Z_AXIS:
            MAP(_MAP_SAVE_SET, Y, Z);
            break;
    }
...
// Z_AXIS is a special mode to apply homing current to all axes.
    ...
    switch (axis) {
        ...
        case X_AXIS:
        case I_AXIS:
            MAP(_MAP_SAVE_SET, X, I);
            break;
        case Y_AXIS:
        case J_AXIS:
            MAP(_MAP_SAVE_SET, Y, J);
            break;
        case Z_AXIS:
            MAP(_MAP_SAVE_SET, Z);
            break;
...
#define _MAP_RESTORE(A) OPTCODE(A##_HAS_HOME_CURRENT, _RESTORE_CURRENT(A))
    ...
    switch (axis) {
        ...
        case X_AXIS:
        case Y_AXIS:
            MAP(_MAP_RESTORE, X, Y);
            break;
        case Z_AXIS:
            MAP(_MAP_RESTORE, Z, Z2, Z3, Z4);
            break;
    }
    ...
    switch (axis) {
        ...
        case X_AXIS:
        case Z_AXIS:
            MAP(_MAP_RESTORE, X, Z);
            break;
        case Y_AXIS:
            MAP(_MAP_RESTORE, Y, Y2);
            break;
    }
    ...
    switch (axis) {
        default:
            break;
        case X_AXIS:
            MAP(_MAP_RESTORE, X, X2);
            break;
        case Y_AXIS:
        case Z_AXIS:
            MAP(_MAP_RESTORE, Y, Z);
            break;
    }
...
// Z_AXIS is a special mode to set homing current to all axes
    ...
    switch (axis) {
        ...
        case X_AXIS:
        case I_AXIS:
            MAP(_MAP_RESTORE, X, I);
            break;
        case Y_AXIS:
        case J_AXIS:
            MAP(_MAP_RESTORE, Y, J);
            break;
        case Z_AXIS:
            MAP(_MAP_RESTORE, Z);
            break;
...
void report_current_position_moving() {
    ...
    SERIAL_ECHOPGM_P(LOGICAL_AXIS_PAIRED_LIST(
        SP_E_LBL, current_position.e, X_LBL, lpos.x, SP_Y_LBL, lpos.y, SP_Z_LBL,
        lpos.z, SP_I_LBL, lpos.i, SP_J_LBL, lpos.j, SP_K_LBL, lpos.k, SP_U_LBL,
        lpos.u, SP_V_LBL, lpos.v, SP_W_LBL, lpos.w));
...
bool position_is_reachable(const float rx, const float ry,
    ...
    can_reach = (R2 <= PRINTABLE_RADIUS_2 - inset
    ...
    const float d1 = rx - (draw_area_min.x), d2 = (draw_area_max.x) - rx,
                y = ry - (draw_area_max.y), a = HYPOT(d1, y), b = HYPOT(d2, y);

    can_reach =
        (a < polargraph_max_belt_len + 1 && b < polargraph_max_belt_len + 1);
...
void sync_plan_position() {
    // if (DEBUGGING(LEVELING)) DEBUG_POS("sync_plan_position",
    // current_position);
    planner.set_position_mm(current_position);
    // SERIAL_ECHOLNPGM("Sync_plan_position: ", current_position.x, ", ",
    // current_position.y, ", ", current_position.z); SERIAL_EOL();
}
...
void _internal_move_to_destination(const feedRate_t fr_mm_s /*=0.0f*/
    ...
    TERN_(HAS_EXTRUDERS,
          REMEMBER(fac, planner.e_factor[active_extruder], 1.0f));
...
void secondary_axis_moves(SECONDARY_AXIS_ARGS_LC(const float),
                          const feedRate_t fr_mm_s) {
    auto move_one = [&](const AxisEnum a, const float p) {
        const feedRate_t fr = fr_mm_s ?: homing_feedrate(a);
        current_position[a] = p;
        line_to_current_position(fr);
    };
    SECONDARY_AXIS_CODE(move_one(I_AXIS, i), move_one(J_AXIS, j),
                        move_one(K_AXIS, k), move_one(U_AXIS, u),
                        move_one(V_AXIS, v), move_one(W_AXIS, w));
}
 ...
 * Plan a move to (X, Y, Z, [I, [J, [K...]]]) with separation of Z from other
 * components.
 *
 ...
 */
void do_blocking_move_to(NUM_AXIS_ARGS_(const float)
...
// DEBUG_SECTION(log_move, "do_blocking_move_to", DEBUGGING(LEVELING));
    ...
    const feedRate_t xy_feedrate =
        fr_mm_s ?: feedRate_t(XY_PROBE_FEEDRATE_MM_S);
    ...
    if (current_position.z > delta_clip_start_height) {
        if (z > delta_clip_start_height) {  // staying in the danger zone
            ...
            prepare_internal_fast_move_to_destination();  // set
                                                          // current_position
                                                          // from destination
            if (DEBUGGING(LEVELING))
        ...
        prepare_internal_fast_move_to_destination();  // set current_position
                                                      // from destination
        if (DEBUGGING(LEVELING))
    ...
    if (z > current_position.z) {  // raising?
        destination.z = z;
        prepare_internal_fast_move_to_destination(
            z_feedrate);  // set current_position from destination
        if (DEBUGGING(LEVELING))
            DEBUG_POS("z raise move", current_position);
    }

    destination.set(x, y);
    prepare_internal_move_to_destination();  // set current_position from
                                             // destination
    if (DEBUGGING(LEVELING))
    ...
    current_position.set(XY_LIST(x, y));
...
void do_blocking_move_to(const xy_pos_t& raw,
                         const feedRate_t fr_mm_s /*=0.0f*/) {
    do_blocking_move_to(NUM_AXIS_LIST_(
        raw.x, raw.y, current_position.z, current_position.i,
        current_position.j, current_position.k, current_position.u,
        current_position.v, current_position.w) fr_mm_s);
}
...
void do_blocking_move_to_x(const float rx, const feedRate_t fr_mm_s /*=0.0*/) {
    // if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPGM("do_blocking_move_to_x(", rx, ",
    // ", fr_mm_s, ")");
    do_blocking_move_to(NUM_AXIS_LIST_(
        rx, current_position.y, current_position.z, current_position.i,
        current_position.j, current_position.k, current_position.u,
        current_position.v, current_position.w) fr_mm_s);
}
#endif
...
void do_blocking_move_to_y(const float ry, const feedRate_t fr_mm_s /*=0.0*/) {
    // if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPGM("do_blocking_move_to_y(", ry, ",
    // ", fr_mm_s, ")");
    do_blocking_move_to(NUM_AXIS_LIST_(
        current_position.x, ry, current_position.z, current_position.i,
        current_position.j, current_position.k, current_position.u,
        current_position.v, current_position.w) fr_mm_s);
}
void do_blocking_move_to_xy(const float rx, const float ry,
                            const feedRate_t fr_mm_s /*=0.0*/) {
    // if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPGM("do_blocking_move_to_xy(", rx,
    // ", ", ry, ", ", fr_mm_s, ")");
    do_blocking_move_to(NUM_AXIS_LIST_(
        rx, ry, current_position.z, current_position.i, current_position.j,
        current_position.k, current_position.u, current_position.v,
        current_position.w) fr_mm_s);
}
...
void do_blocking_move_to_z(const float rz, const feedRate_t fr_mm_s /*=0.0*/) {
    // if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPGM("do_blocking_move_to_z(", rz, ",
    // ", fr_mm_s, ")");
    do_blocking_move_to_xy_z(current_position, rz, fr_mm_s);
}
void do_blocking_move_to_xy_z(const xy_pos_t& raw, const float z,
                              const feedRate_t fr_mm_s /*=0.0f*/) {
    do_blocking_move_to(NUM_AXIS_LIST_(raw.x, raw.y, z, current_position.i,
                                       current_position.j, current_position.k,
                                       current_position.u, current_position.v,
                                       current_position.w) fr_mm_s);
}
/**
 ...
 */
void do_z_clearance(const float zclear, const bool with_probe /*=true*/,
                    const bool lower_allowed /*=false*/) {
    UNUSED(with_probe);
    float zdest = zclear;
    TERN_(HAS_BED_PROBE,
          if (with_probe && probe.offset.z < 0) zdest -= probe.offset.z);
    NOMORE(zdest, Z_MAX_POS);
    // if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPGM("do_z_clearance(", zclear, " [",
    // current_position.z, " to ", zdest, "], ", lower_allowed, ")");
    if ((!lower_allowed && zdest < current_position.z) ||
        zdest == current_position.z)
        return;
    do_blocking_move_to_z(
        zdest, TERN(HAS_BED_PROBE, z_probe_fast_mm_s, homing_feedrate(Z_AXIS)));
}
...
// DEBUG_SECTION(mzah, "do_move_after_z_homing", DEBUGGING(LEVELING));
    ...
    do_z_clearance(Z_POST_CLEARANCE,
                   ALL(HOMING_Z_WITH_PROBE, HAS_STOWABLE_PROBE) &&
                       TERN0(HAS_BED_PROBE, endstops.z_probe_enabled),
                   true);
...
#if HAS_I_AXIS
void do_blocking_move_to_xyz_i(const xyze_pos_t& raw, const float i,
                               const feedRate_t fr_mm_s /*=0.0f*/) {
    do_blocking_move_to(NUM_AXIS_LIST_(raw.x, raw.y, raw.z, i, raw.j, raw.k,
                                       raw.u, raw.v, raw.w) fr_mm_s);
}
...
#if HAS_J_AXIS
void do_blocking_move_to_xyzi_j(const xyze_pos_t& raw, const float j,
                                const feedRate_t fr_mm_s /*=0.0f*/) {
    do_blocking_move_to(NUM_AXIS_LIST_(raw.x, raw.y, raw.z, raw.i, j, raw.k,
                                       raw.u, raw.v, raw.w) fr_mm_s);
}
...
#if HAS_K_AXIS
void do_blocking_move_to_xyzij_k(const xyze_pos_t& raw, const float k,
                                 const feedRate_t fr_mm_s /*=0.0f*/) {
    do_blocking_move_to(NUM_AXIS_LIST_(raw.x, raw.y, raw.z, raw.i, raw.j, k,
                                       raw.u, raw.v, raw.w) fr_mm_s);
}
...
#if HAS_U_AXIS
void do_blocking_move_to_xyzijk_u(const xyze_pos_t& raw, const float u,
                                  const feedRate_t fr_mm_s /*=0.0f*/) {
    do_blocking_move_to(NUM_AXIS_LIST_(raw.x, raw.y, raw.z, raw.i, raw.j, raw.k,
                                       u, raw.v, raw.w) fr_mm_s);
}
...
#if HAS_V_AXIS
void do_blocking_move_to_xyzijku_v(const xyze_pos_t& raw, const float v,
                                   const feedRate_t fr_mm_s /*=0.0f*/) {
    do_blocking_move_to(NUM_AXIS_LIST_(raw.x, raw.y, raw.z, raw.i, raw.j, raw.k,
                                       raw.u, v, raw.w) fr_mm_s);
}
...
#if HAS_W_AXIS
void do_blocking_move_to_xyzijkuv_w(const xyze_pos_t& raw, const float w,
                                    const feedRate_t fr_mm_s /*=0.0f*/) {
    do_blocking_move_to(NUM_AXIS_LIST_(raw.x, raw.y, raw.z, raw.i, raw.j, raw.k,
                                       raw.u, raw.v, w) fr_mm_s);
}
...
void remember_feedrate_scaling_off() {
    // if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPGM("remember_feedrate_scaling_off:
    // fr=", feedrate_mm_s, " ", feedrate_percentage, "%");
...
void restore_feedrate_and_scaling() {
    ...
    // if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPGM("restore_feedrate_and_scaling:
    // fr=", feedrate_mm_s, " ", feedrate_percentage, "%");
}
 ...
 */
void update_software_endstops(const AxisEnum axis OPTARG(
    HAS_HOTEND_OFFSET, const uint8_t old_tool_index /*=0*/,
    ...
    if (axis == X_AXIS) {
        ...
        if (new_tool_index != 0) {
            // T1 can move from X2_MIN_POS to X2_MAX_POS or X2 home position
            // (whichever is larger)
            ...
            soft_endstop.max.x =
                _MIN(X1_MAX_POS, dual_max_x - duplicate_extruder_x_offset);
        } else {
    ...
    switch (axis) {
        ...
        case Y_AXIS:
            ...
            delta_max_radius =
                _MIN(ABS(_MAX(soft_endstop.min.x, soft_endstop.min.y)),
                     soft_endstop.max.x, soft_endstop.max.y);
    ...
    if (new_tool_index == old_tool_index ||
        axis == Z_AXIS) {  // The Z axis is "special" and shouldn't be modified
        const float offs =
            (axis == Z_AXIS) ? 0 : hotend_offset[active_extruder][axis];
        soft_endstop.min[axis] = base_min_pos(axis) + offs;
        soft_endstop.max[axis] = base_max_pos(axis) + offs;
    } else {
        const float diff = hotend_offset[new_tool_index][axis] -
                           hotend_offset[old_tool_index][axis];
    ...
    // if (DEBUGGING(LEVELING))
    // SERIAL_ECHOLNPGM("Axis ", C(AXIS_CHAR(axis)), " min:",
    // soft_endstop.min[axis], " max:", soft_endstop.max[axis]);
}
...
void apply_motion_limits(xyz_pos_t& target) {
    // SERIAL_ECHOLNPGM("Motion limits in: ", target.x, ", ", target.y, ", ",
    // target.z); SERIAL_EOL();
...
#else
    // SCARA needs to consider the angle of the arm through the entire move, so
    // for now use no tool offset.
    ...
    if (TERN1(IS_SCARA, axis_was_homed(X_AXIS) && axis_was_homed(Y_AXIS))) {
        ...
        // SERIAL_ECHOLNPGM("Motion limits data: dist_2:", dist_2, "
        // delta_max_radius_2: ", delta_max_radius_2); SERIAL_EOL();
        if (dist_2 > delta_max_radius_2)
            target *=
                float(delta_max_radius / SQRT(dist_2));  // 200 / 300 = 0.66
    }
#endif
...
FORCE_INLINE void segment_idle(millis_t& next_idle_ms) {
    ...
    if (ELAPSED(ms, next_idle_ms)) {
        ...
        return marlin_wrapper_idle();
    }
}

/**
 * Get distance from displacements along axes and, if required, update move
 * type.
 */
float get_move_distance(const xyze_pos_t& diff OPTARG(
    HAS_ROTATIONAL_AXES, bool& is_cartesian_move)) {
#if NUM_AXES

    if (NUM_AXIS_NONE(diff.x, diff.y, 0, diff.i, diff.j, diff.k, diff.u, diff.v,
                      diff.w))
        return TERN0(HAS_Z_AXIS, ABS(diff.z));

#if ENABLED(ARTICULATED_ROBOT_ARM)

    // For articulated robots, interpreting feedrate like LinuxCNC would require
    // inverse kinematics. As a workaround, pretend that motors sit on n
    // mutually orthogonal axes and assume that we could think of distance as
    // magnitude of an n-vector in an n-dimensional Euclidian space.
    const float distance_sqr = NUM_AXIS_GANG(
        sq(diff.x), +sq(diff.y), +sq(diff.z), +sq(diff.i), +sq(diff.j),
        +sq(diff.k), +sq(diff.u), +sq(diff.v), +sq(diff.w));

#elif ENABLED(FOAMCUTTER_XYUV)

    const float distance_sqr = (
#if HAS_J_AXIS
        _MAX(sq(diff.x) + sq(diff.y),
             sq(diff.i) + sq(diff.j))  // Special 5 axis kinematics. Return the
                                       // larger of plane X/Y or I/J
#else
    ...
    /**
     * Calculate distance for feedrate interpretation in accordance with NIST
     * RS274NGC interpreter - version 3) and its default CANON_XYZ feed
     * reference mode. Assume:
     ...
     *
     *   D^2 = dX^2 + dY^2 + dZ^2
     *   if D^2 == 0 (none of XYZ move but any secondary linear axes move,
     * whether other axes are moved or not): D^2 = dU^2 + dV^2 + dW^2 if D^2 ==
     * 0 (only rotational axes are moved): D^2 = dA^2 + dB^2 + dC^2
     */
    float distance_sqr = XYZ_GANG(sq(diff.x), +sq(diff.y), +sq(diff.z));

#if SECONDARY_LINEAR_AXES
    if (UNEAR_ZERO(distance_sqr)) {
        // Move does not involve any primary linear axes (xyz) but might involve
        // secondary linear axes
        distance_sqr =
            (SECONDARY_AXIS_GANG(IF_DISABLED(AXIS4_ROTATES, +sq(diff.i)),
    ...
    if (UNEAR_ZERO(distance_sqr)) {
        // Move involves no linear axes. Calculate angular distance in
        // accordance with LinuxCNC
        distance_sqr =
            ROTATIONAL_AXIS_GANG(sq(diff.i), +sq(diff.j), +sq(diff.k),
...
inline bool line_to_destination_kinematic() {
    ...
    // SERIAL_ECHOLNPGM("Difference : ", diff.x, " , ", diff.y, " , ", diff.z, "
    // , ", diff.e);
...
#endif
    float cartesian_mm =
        get_move_distance(diff OPTARG(HAS_ROTATIONAL_AXES, cartes_move));
 ...
 */
inline void segmented_line_to_destination(
    const feedRate_t fr_mm_s,
...
inline bool line_to_destination_cartesian() {
    ...
    if (planner.leveling_active &&
        planner.leveling_active_at_z(destination.z)) {
#if ENABLED(AUTO_BED_LEVELING_UBL)
#if UBL_SEGMENTED
        return bedlevel.line_to_destination_segmented(scaled_fr_mm_s);
#else
        bedlevel.line_to_destination_cartesian(
            scaled_fr_mm_s,
            active_extruder);  // UBL's motion routine needs to know about
...
float inactive_extruder_x = X2_MAX_POS,  // Used in mode 0 & 1
    duplicate_extruder_x_offset =
        DEFAULT_DUPLICATION_X_OFFSET;          // Used in mode 2 & 3
...
void idex_set_parked(const bool park /*=true*/) {
    ...
    if (park)
        raised_parked_position =
            current_position;  // Remember current raised toolhead position for
                               // use by unpark
}
        ...
        switch (dual_x_carriage_mode) {
            ...
            case DXC_AUTO_PARK_MODE: {
                if (current_position.e == destination.e) {
                    ...
                    // (so it can be used as the start of the next non-travel
                    // move)
                ...
                //
                const feedRate_t fr_zfast =
                    planner.settings.max_feedrate_mm_s[Z_AXIS];
                //  1. Move to the raised parked XYZ. Presumably the tool is
                //  already at XY.
                xyze_pos_t raised = raised_parked_position;
                raised.e = current_position.e;
                if (planner.buffer_line(raised, fr_zfast)) {
                    //  2. Move to the current native XY and raised Z.
                    //  Presumably this is a null move.
            ...
            case DXC_DUPLICATION_MODE:
                if (active_extruder == 0) {
                    set_duplication_enabled(
                        false);  // Clear stale duplication state
                    ...
                    if (DEBUGGING(LEVELING))
                        DEBUG_ECHOLNPGM("Set planner X", inactive_extruder_x,
                                        " ... Line to X", new_pos.x);
                    if (!planner.buffer_line(
                            new_pos, planner.settings.max_feedrate_mm_s[X_AXIS],
                            1))
                        break;
                    planner.synchronize();
...
void prepare_line_to_destination() {
    ...
    if (!DEBUGGING(DRYRUN) && destination.e != current_position.e) {
        ...
        const float e_delta = ABS(destination.e - current_position.e) *
                              planner.e_factor[active_extruder];
        ...
        if (ignore_e) {
            current_position.e =
                destination.e;  // Behave as if the E move really took place
            planner.set_e_position_mm(
                destination.e);  // Prevent the planner from complaining too
        }
    }
    ...
    if (
#if UBL_SEGMENTED
#if IS_KINEMATIC  // UBL using Kinematic / Cartesian cases as a workaround for
                  // now.
...
main_axes_bits_t axes_should_home(
    main_axes_bits_t axis_bits /*=main_axes_mask*/) {
    auto set_should = [](main_axes_bits_t& b, AxisEnum a) {
        if (TEST(b, a) &&
            TERN(HOME_AFTER_DEACTIVATE, axis_is_trusted, axis_was_homed)(a))
            CBI(b, a);
    };
    // Clear test bits that are trusted
    NUM_AXIS_CODE(set_should(axis_bits, X_AXIS), set_should(axis_bits, Y_AXIS),
                  set_should(axis_bits, Z_AXIS), set_should(axis_bits, I_AXIS),
                  set_should(axis_bits, J_AXIS), set_should(axis_bits, K_AXIS),
                  set_should(axis_bits, U_AXIS), set_should(axis_bits, V_AXIS),
...
void end_sensorless_homing_per_axis(const AxisEnum axis,
                                    sensorless_t enable_stealth) {
    switch (axis) {
        default:
            break;
#if X_SENSORLESS
...
void do_homing_move(const AxisEnum axis, const float distance,
    ...
    if (is_home_dir) {
        if (TERN0(HOMING_Z_WITH_PROBE, axis == Z_AXIS)) {
            ...
            TERN_(
                HAS_QUIET_PROBING,
                if (final_approach) probe.set_devices_paused_for_probing(true));
        }
...
// Disable stealthChop if used. Enable diag1 pin on driver.
...
#if SENSORLESS_STALLGUARD_DELAY
        safe_delay(
            SENSORLESS_STALLGUARD_DELAY);  // Short delay needed to settle
#endif
#endif
    }
    ...
    if (is_home_dir) {
        ...
        if (axis == Z_AXIS && final_approach)
            probe.set_devices_paused_for_probing(false);
#endif
...
/**
 * Set an axis to be unhomed. (Unless we are on a machine - e.g. a cheap Chinese
 * CNC machine - that has no endstops. Such machines should always be considered
 * to be in a "known" and "trusted" position).
 */
void set_axis_never_homed(const AxisEnum axis) {
    // if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPGM(">>> set_axis_never_homed(",
    // C(AXIS_CHAR(axis)), ")");
...
void backout_to_tmc_homing_phase(const AxisEnum axis) {
    ...
    int16_t phasePerUStep,  // TMC µsteps(phase) per Marlin µsteps
        ...
        effectorBackoutDir,  // Direction in which the effector mm coordinates
                             // move away from endstop.
        stepperBackoutDir;   // Direction in which the TMC µstep count(phase)
                             // move away from endstop.
...
#define PHASE_PER_MICROSTEP(N) (256 / _MAX(1, N##_MICROSTEPS))
    ...
    // Phase distance to nearest home phase position when moving in the backout
    // direction from endstop (may be negative).
    int16_t phaseDelta = (home_phase[axis] - phaseCurrent) * stepperBackoutDir;

    // Check if home distance within endstop assumed repeatability noise of
    // .05mm and warn.
    ...
    // Skip to next if target position is behind current. So it only moves away
    // from endstop.
    if (phaseDelta < 0)
        phaseDelta += 1024;

    // Convert TMC µsteps (phase) to whole Marlin µsteps to effector backout
    // direction to mm
    const float mmDelta = int16_t(phaseDelta / phasePerUStep) *
                          effectorBackoutDir * planner.mm_per_step[axis];

    // Optional debug messages
    if (DEBUGGING(LEVELING)) {
        DEBUG_ECHOLNPGM("Endstop ", C(AXIS_CHAR(axis)),
                        " hit at Phase:", phaseCurrent, " Delta:", phaseDelta,
                        " Distance:", mmDelta);
    }
...
#else
#define _CAN_HOME(A)                                                  \
    (axis == _AXIS(A) && (ANY(A##_SPI_SENSORLESS, HAS_##A##_STATE) || \
                          TERN0(HOMING_Z_WITH_PROBE, _AXIS(A) == Z_AXIS)))
#define _ANDCANT(N) &&!_CAN_HOME(N)
    if (true MAIN_AXIS_MAP(_ANDCANT))
        return;
#endif

    // if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPGM(">>> homeaxis(",
    // C(AXIS_CHAR(axis)), ")");
...
//
    ...
    if ((TERN0(X_SENSORLESS, axis == X_AXIS) ||
         TERN0(Y_SENSORLESS, axis == Y_AXIS) ||
         TERN0(Z_SENSORLESS, axis == Z_AXIS) ||
         TERN0(I_SENSORLESS, axis == I_AXIS) ||
         TERN0(J_SENSORLESS, axis == J_AXIS) ||
         TERN0(K_SENSORLESS, axis == K_AXIS)) &&
        backoff[axis]) {
...
//
    ...
    if (!(axis_was_homed(X_AXIS) || axis_was_homed(Y_AXIS)) &&
        ...
        const float backoff_length =
            -ABS(XY_COUNTERPART_BACKOFF_MM) * home_dir(opposite_axis);
        do_homing_move(opposite_axis, backoff_length,
                       homing_feedrate(opposite_axis));
    }
#endif

    // Determine if a homing bump will be done and the bumps distance
    // When homing Z with probe respect probe clearance
    const bool use_probe_bump =
        TERN0(HOMING_Z_WITH_PROBE, axis == Z_AXIS && home_bump_mm(axis));
    const float bump =
        axis_home_dir *
        (use_probe_bump
             ? _MAX(TERN0(HOMING_Z_WITH_PROBE, Z_CLEARANCE_BETWEEN_PROBES),
                    home_bump_mm(axis))
             : home_bump_mm(axis));
    ...
    //
    const float move_length =
        1.5f * max_length(TERN(DELTA, Z_AXIS, axis)) * axis_home_dir;
    // if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPGM("Home Fast: ", move_length,
    // "mm");
    TRACE_PRINTF("Home Fast: %fmm at %fmm/s\n", move_length,
    ...
    if (bump) {
        ...
        do_homing_move(axis, -bump,
                       TERN0(HOMING_Z_WITH_PROBE,
                             (axis == Z_AXIS ? z_probe_fast_mm_s : 0)),
                       false);
...
#define _ESCASE(A)        \
    case A##_AXIS:        \
        ...
        if (axis == X_AXIS && axis_home_dir > 0) {
            es = X_MAX;
            if (DEBUGGING(LEVELING))
                DEBUG_ECHOLNPGM("DUAL_X_CARRIAGE: Homing to X_MAX");
        }
#endif
    ...
    if (axis == X_AXIS) {
        ...
        if (adj) {
            if (pos_dir ? (endstops.x2_endstop_adj > 0)
                        : (endstops.x2_endstop_adj < 0))
    ...
    if (axis == Y_AXIS) {
        ...
        if (adj) {
            if (pos_dir ? (endstops.y2_endstop_adj > 0)
                        : (endstops.y2_endstop_adj < 0))
    ...
    if (axis == Z_AXIS) {
        ...
        if (adj) {
            if (pos_dir ? (endstops.z2_endstop_adj > 0)
                        : (endstops.z2_endstop_adj < 0))
        ...
        adjustFunc_t lock[] =
            ARRAY_N(NUM_Z_STEPPERS, stepper.set_z1_lock, stepper.set_z2_lock,
                    stepper.set_z3_lock, stepper.set_z4_lock);
        float adj[] = ARRAY_N(NUM_Z_STEPPERS, 0, endstops.z2_endstop_adj,
                              endstops.z3_endstop_adj, endstops.z4_endstop_adj);
        ...
        CODE_N(NUM_Z_STEPPERS, stepper.set_z1_lock(false),
               stepper.set_z2_lock(false), stepper.set_z3_lock(false),
    ...
    // Retrace by the amount specified in delta_endstop_adj if more than min
    // steps.
    if (adjDistance * (Z_HOME_DIR) < 0 &&
        ABS(adjDistance) >
            minDistance) {  // away from endstop, more than min distance
        if (DEBUGGING(LEVELING))
    ...
    // if (DEBUGGING(LEVELING)) DEBUG_POS("> AFTER set_axis_is_at_home",
    // current_position);
    ...
    if (TERN0(HOMING_Z_WITH_PROBE, axis == Z_AXIS && probe.stow()))
        return;
    ...
    if (endstop_backoff[axis]) {
        ...
        line_to_current_position(
            TERN_(HOMING_Z_WITH_PROBE, (axis == Z_AXIS) ? z_probe_fast_mm_s :)
                homing_feedrate(axis));
...
void set_axis_is_at_home(const AxisEnum axis) {
    // if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPGM(">>> set_axis_is_at_home(",
    // C(AXIS_CHAR(axis)), ")");
...
#if ENABLED(DUAL_X_CARRIAGE)
    if (axis == X_AXIS && (active_extruder == 1 ||
                           dual_x_carriage_mode == DXC_DUPLICATION_MODE)) {
        current_position.x = SUM_TERN(
            HAS_HOME_OFFSET, x_home_pos(active_extruder), home_offset.x);
        return;
    }
#endif
...
#else
    current_position[axis] =
        SUM_TERN(HAS_HOME_OFFSET, base_home_pos(axis), home_offset[axis]);
#endif
 ...
 */
    ...
    // if (DEBUGGING(LEVELING)) {
    //   #if HAS_HOME_OFFSET
    //     DEBUG_ECHOLNPGM("> home_offset[", C(AXIS_CHAR(axis)), "] = ",
    //     home_offset[axis]);
    //   #endif
