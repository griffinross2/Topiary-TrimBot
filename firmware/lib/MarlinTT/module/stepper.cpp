 ...
 */
  ...
  uint32_t Stepper::time_spent_in_isr = 0, Stepper::time_spent_out_isr = 0;
#endif
...
constexpr uint32_t ns_to_pulse_timer_ticks(const uint32_t ns) { return (ns + ns_per_pulse_timer_tick / 2) / ns_per_pulse_timer_tick; }

constexpr uint32_t timer_setup_ns = cycles_to_ns(timer_read_add_and_store_cycles);
constexpr uint32_t PULSE_HIGH_TICK_COUNT = ns_to_pulse_timer_ticks(_min_pulse_high_ns - _MIN(_min_pulse_high_ns, timer_setup_ns));
constexpr uint32_t PULSE_LOW_TICK_COUNT = ns_to_pulse_timer_ticks(_min_pulse_low_ns - _MIN(_min_pulse_low_ns, timer_setup_ns));
...
void Stepper::isr() {
  ...
  #endif
  #if ENABLED(SMOOTH_LIN_ADVANCE)
    static uint32_t smoothLinAdvISR = 0;       // Interval until the next Smooth Linear Advance phase (0 = Now)
  #endif
  #if ENABLED(FT_MOTION)
  ...
  do {
    ...
    uint32_t interval = 0;
      ...
      if (!using_ftMotion) {
        ...
        interval = uint32_t(STEPPER_TIMER_RATE * 0.03);                  // Max wait of 30ms regardless of stepper timer frequency
     ...
     */
    min_ticks = marlin_wrapper_step_timer_count() + uint32_t(TERN(__AVR__, 8, 1) * (STEPPER_TIMER_TICKS_PER_US));
    ...
    const uint32_t time_spent = marlin_wrapper_step_timer_count();
  ...
  uint32_t Stepper::calc_multistep_timer_interval(uint32_t step_rate) {
  ...
  uint32_t Stepper::block_phase_isr() {
    ...
    uint32_t Stepper::smooth_lin_adv_isr() {
 ...
 */
  ...
  #define _APPLY_DIR(AXIS, FWD) AXIS ##_APPLY_DIR(FWD, true)
    ...
    #define _SAVE_START() const uint32_t pulse_start = HAL_timer_get_count(MF_TIMER_PULSE)
