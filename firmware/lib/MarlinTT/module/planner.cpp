...
void Planner::finish_and_disable() {
  while (has_blocks_queued() || cleaning_buffer_counter) marlin_wrapper_idle();
 ...
 */
void Planner::synchronize() { while (busy()) marlin_wrapper_idle(); }
...
inline void limit_and_warn(float &val, const AxisEnum axis, const char* setting_name, const xyze_float_t &max_limit) {
  const uint8_t lim_axis = TERN_(HAS_EXTRUDERS, axis > E_AXIS ? E_AXIS :) axis;
  const float before = val;
  LIMIT(val, 0.1f, max_limit[lim_axis]);
  if (before != val)
    printf("%c Max %s limited to %.1f\n", AXIS_CHAR(lim_axis), setting_name, val);
}
