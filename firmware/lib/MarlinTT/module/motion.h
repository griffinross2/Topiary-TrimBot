...
#if DISABLED(DELTA)
    NUM_AXIS_CODE(if (a == X_AXIS) v = homing_feedrate_mm_m.x,
...
#define XYZ_DEFS(T, NAME, OPT)                                          \
    inline T NAME(const AxisEnum axis) {                                \
        static constexpr XYZval<T> NAME##_P =                           \
            NUM_AXIS_ARRAY(X_##OPT, Y_##OPT, Z_##OPT, I_##OPT, J_##OPT, \
                           K_##OPT, U_##OPT, V_##OPT, W_##OPT);         \
        return NAME##_P[axis];                                          \
    }
...
constexpr AxisFlags rotational{0 LOGICAL_AXIS_GANG(
    | 0, | 0, | 0, | 0, | (ENABLED(AXIS4_ROTATES) << I_AXIS),
    | (ENABLED(AXIS5_ROTATES) << J_AXIS), | (ENABLED(AXIS6_ROTATES) << K_AXIS),
    | (ENABLED(AXIS7_ROTATES) << U_AXIS), | (ENABLED(AXIS8_ROTATES) << V_AXIS),
...
struct PositionReport {
    static void report() {
        TERN(AUTO_REPORT_REAL_POSITION, report_real_position(),
...
float get_move_distance(const xyze_pos_t& diff OPTARG(HAS_ROTATIONAL_AXES,
                                                      bool& is_cartesian_move));
...
void _internal_move_to_destination(
    const feedRate_t fr_mm_s = 0.0f OPTARG(IS_KINEMATIC,
...
void do_blocking_move_to_xy_z(const xy_pos_t& raw, const float z,
                              const feedRate_t fr_mm_s = 0.0f);
FORCE_INLINE void do_blocking_move_to_xy_z(const xyz_pos_t& raw, const float z,
                                           const feedRate_t fr_mm_s = 0.0f) {
    do_blocking_move_to_xy_z(xy_pos_t(raw), z, fr_mm_s);
}
FORCE_INLINE void do_blocking_move_to_xy_z(const xyze_pos_t& raw, const float z,
                                           const feedRate_t fr_mm_s = 0.0f) {
    do_blocking_move_to_xy_z(xy_pos_t(raw), z, fr_mm_s);
}
#endif
...
inline void do_z_post_clearance() {
...
#if HAS_Z_AXIS
extern bool z_min_trusted;  // If Z has been powered on trust that the real Z is
                            // >= current_position.z
#endif
...
#else
constexpr main_axes_bits_t
    axes_homed = main_axes_mask,
    axes_trusted =
        main_axes_mask;  // Zero-endstop machines are always homed and trusted
...
inline void set_axis_unhomed(const AxisEnum axis) {
    TERN_(HAS_ENDSTOPS, CBI(axes_homed, axis));
}
inline void set_axis_untrusted(const AxisEnum axis) {
    TERN_(HAS_ENDSTOPS, CBI(axes_trusted, axis));
}
inline void set_all_unhomed() {
    TERN_(HAS_ENDSTOPS, axes_homed = axes_trusted = 0);
}
inline void set_axis_homed(const AxisEnum axis) {
    TERN_(HAS_ENDSTOPS, SBI(axes_homed, axis));
}
inline void set_axis_trusted(const AxisEnum axis) {
    TERN_(HAS_ENDSTOPS, SBI(axes_trusted, axis));
}
inline void set_all_homed() {
    TERN_(HAS_ENDSTOPS, axes_homed = axes_trusted = main_axes_mask);
}
...
#define MOTION_CONDITIONS \
    (marlin_wrapper_is_running() && !homing_needed_error())
#else
#define MOTION_CONDITIONS marlin_wrapper_is_running()
#endif

#define BABYSTEP_ALLOWED()                                       \
    ((ENABLED(BABYSTEP_WITHOUT_HOMING) || all_axes_trusted()) && \
     (ENABLED(BABYSTEP_ALWAYS_AVAILABLE) || marlin_wrapper_is_printer_busy()))
...
FORCE_INLINE void toLogical(xy_pos_t& raw) {
    raw += workspace_offset;
}
FORCE_INLINE void toLogical(xyz_pos_t& raw) {
    raw += workspace_offset;
}
FORCE_INLINE void toLogical(xyze_pos_t& raw) {
    raw += workspace_offset;
}
...
FORCE_INLINE void toNative(xyze_pos_t& raw) {
    raw -= workspace_offset;
}
#else
...
FORCE_INLINE void set_duplication_enabled(const bool dupe) {
