...
#define XYZ_DEFS(T, NAME, OPT) \
  inline T NAME(const AxisEnum axis) { \
    static constexpr XYZval<T> NAME##_P = NUM_AXIS_ARRAY(X_##OPT, Y_##OPT, Z_##OPT, I_##OPT, J_##OPT, K_##OPT, U_##OPT, V_##OPT, W_##OPT); \
    return NAME##_P[axis]; \
  }
...
void home_if_needed(const bool keeplev=false);
  ...
  #define MOTION_CONDITIONS (marlin_wrapper_is_running() && !homing_needed_error())
#else
  #define MOTION_CONDITIONS marlin_wrapper_is_running()
#endif

#define BABYSTEP_ALLOWED() ((ENABLED(BABYSTEP_WITHOUT_HOMING) || all_axes_trusted()) && (ENABLED(BABYSTEP_ALWAYS_AVAILABLE) || marlin_wrapper_is_printer_busy()))
