...
#include "vector_3.h"   // for matrix_3x3
...
static const uint32_t   _DMA[] = DEFAULT_MAX_ACCELERATION;
static const feedRate_t _DMF[] = DEFAULT_MAX_FEEDRATE;
...
void MarlinSettings::reset() {
  LOOP_DISTINCT_AXES(i) {
    planner.settings.max_acceleration_mm_per_s2[i] = _DMA[ALIM(i, _DMA)];
    #if ENABLED(EDITABLE_STEPS_PER_UNIT)
      planner.settings.axis_steps_per_mm[i] = _DASU[ALIM(i, _DASU)];
    #endif
    planner.settings.max_feedrate_mm_s[i] = _DMF[ALIM(i, _DMF)];
  }
