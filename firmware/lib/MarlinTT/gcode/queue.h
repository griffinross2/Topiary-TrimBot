...
public:
   ...
   */
  static bool has_commands_queued() { return ring_buffer.length; }
   ...
   */
  static void set_current_line_number(long n) { serial_state.last_N = n; }

  /**
   * Get the current line number for the last received command
   */
  static long get_current_line_number() { return serial_state.last_N; }
...
private:
  ...
  static void gcode_line_error(const char* ferr);
