...
inline int read_serial() {
    return marlin_wrapper_serial_read();
}
...
/**
 * arduinoststm32's USB receive buffer is not well behaved when the buffer
 * overflows
 *
...
void GCodeQueue::flush_rx() {
    ...
    for (uint8_t p = 0; p < NUM_SERIAL; ++p) {
        if (!serial_data_available(p))
            continue;  // No data for this port? Skip.
        while (SERIAL_IMPL.available(p))
            (void)read_serial(p);
    }
}
...
inline void process_stream_char(const char c, uint8_t& sis,
                                char (&buff)[MAX_CMD_SIZE], int& ind) {
    if (sis == PS_EOL)
 ...
 */
inline bool process_line_done(uint8_t& sis, char (&buff)[MAX_CMD_SIZE],
                              int& ind) {
...
void GCodeQueue::get_serial_commands() {
    ...
    if (card.flag.binary_mode) {
        /**
         * For binary stream file transfer, use serial_line_buffer as the
         * working receive buffer (which limits the packet size to
         * MAX_CMD_SIZE). The receive buffer also limits the packet size for
         * reliable transmission.
         */
        binaryStream[card.transfer_port_index.index].receive(
            serial_state[card.transfer_port_index.index].line_buffer);
        return;
    }
#endif
...
// send "wait" to indicate Marlin is still waiting.
    ...
    if (ring_buffer.empty() && !any_serial_data_available() &&
    ...
    for (bool hadData = true; hadData;) {
        ...
        if (ISEOL(serial_char)) {
            ...
            if (process_line_done(serial.input_state, serial.line_buffer,
                                  serial.count))
                continue;
            ...
            char* npos =
                (*command == 'N')
                    ? command
                    : nullptr;  // Require the N parameter to start the line
            ...
            if (npos) {
                ...
                if (gcode_N != serial.last_N + 1 && !M110) {
                    // A request-for-resend line was already in transit so we
                    // got two - oops!
            ...
            ring_buffer.enqueue(serial.line_buffer,
                                false OPTARG(HAS_MULTI_SERIAL, p));
        } else
            process_stream_char(serial_char, serial.input_state,
                                serial.line_buffer, serial.count);
...
inline void GCodeQueue::get_sdcard_commands() {
    ...
    while (!ring_buffer.full() && !card.eof()) {
        ...
        if (is_eol || card_eof) {
            // Reset stream state, terminate the buffer, and commit a non-empty
            // command
            if (!is_eol && sd_count)
                ++sd_count;  // End of file with no newline
            if (!process_line_done(sd_input_state, command.buffer, sd_count)) {
                // M808 L saves the sdpos of the next line. M808 loops to a new
                // sdpos.
                TERN_(GCODE_REPEAT_MARKERS,
                      repeat.early_parse_M808(command.buffer));

#if DISABLED(PARK_HEAD_ON_PAUSE)
                // When M25 is non-blocking it can still suspend SD commands
                // Otherwise the M125 handler needs to know SD printing is
                // active
                if (command.buffer[0] == 'M' && command.buffer[1] == '2' &&
                    command.buffer[2] == '5' && !NUMERIC(command.buffer[3]))
                    card.pauseSDPrint();
#endif
                ...
                TERN_(POWER_LOSS_RECOVERY,
                      recovery.cmd_sdpos = card.getIndex());
            }

            if (card.eof())
                card.fileHasFinished();  // Handle end of file reached
        } else
            process_stream_char(sd_char, sd_input_state, command.buffer,
                                sd_count);
    }
}
...
void GCodeQueue::advance() {
    ...
    if (command_buffer_empty) {
        ...
        const millis_t command_buffer_empty_duration =
            millis() - command_buffer_empty_at;
        NOLESS(max_command_buffer_empty_duration,
               command_buffer_empty_duration);
    }
#endif
    ...
    if (card.flag.saving) {
        ...
        if (is_M29(cmd)) {
            ...
            if (card.flag.logging)
                gcode.process_next_command();  // The card is saving because
                                               // it's logging
            else
                ok_to_send();
        }
    ...
    // The queue may be reset by a command handler or by code invoked by idle()
    // within a handler
...
void GCodeQueue::report_buffer_statistics() {
    SERIAL_ECHOLNPGM(
        "D576"
        " P:",
        planner.moves_free(), " ", planner_buffer_underruns, " (",
        max_planner_buffer_empty_duration,
        ")"
        " B:",
        BUFSIZE - ring_buffer.length, " ", command_buffer_underruns, " (",
...
void GCodeQueue::auto_report_buffer_statistics() {
    // Bit of a hack to try to catch planner buffer underruns without having
    // logic running inside Stepper::block_phase_isr
    const millis_t ms = millis();
    if (planner.movesplanned() == 0) {
        if (!planner_buffer_empty) {  // the planner buffer wasn't empty, but
                                      // now it is
            planner_buffer_empty = true;
            planner_buffer_underruns++;
            planner_buffer_empty_at = ms;
        }
    } else if (planner_buffer_empty) {  // the planner buffer was empty, but now
                                        // it's not
        planner_buffer_empty = false;
        const millis_t planner_buffer_empty_duration =
            ms - planner_buffer_empty_at;
        NOLESS(max_planner_buffer_empty_duration,
               planner_buffer_empty_duration);  // if it's longer than the
                                                // currently tracked max
                                                // duration, replace it
    }
