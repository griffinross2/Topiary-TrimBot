...
static void quick_home_xy() {
    ...
    sensorless_t stealth_states{
        NUM_AXIS_LIST(TERN0(X_SENSORLESS, tmc_enable_stallguard(stepperX)),
    ...
    do_blocking_move_to_xy(1.5 * max_length(X_AXIS) * x_axis_home_dir,
                           1.5 * max_length(Y_AXIS) * Y_HOME_DIR, fr_mm_s);
...
inline void home_z_safely() {
     ...
     */
    constexpr xy_float_t safe_homing_xy = {Z_SAFE_HOMING_X_POINT,
...
motion_state_t begin_slow_homing() {
    ...
    motion_state.acceleration.set(
        ...
        planner.settings.max_acceleration_mm_per_s2[Y_AXIS] OPTARG(
            DELTA, planner.settings.max_acceleration_mm_per_s2[Z_AXIS]));
...
void end_slow_homing(const motion_state_t& motion_state) {
    planner.settings.max_acceleration_mm_per_s2[X_AXIS] =
        motion_state.acceleration.x;
    planner.settings.max_acceleration_mm_per_s2[Y_AXIS] =
        motion_state.acceleration.y;
    TERN_(DELTA, planner.settings.max_acceleration_mm_per_s2[Z_AXIS] =
                     motion_state.acceleration.z);
 ...
 *
 *  L<bool>   Force leveling state ON (if possible) or OFF after homing
 * (Requires RESTORE_LEVELING_AFTER_G28 or ENABLE_LEVELING_AFTER_G28) O Home
 * only if the position is not known and trusted R<linear> Raise by n mm/inches
 * before homing H         Hold the current X/Y position when executing a home
 * Z, or if multiple axes are homed, the position when Z home is executed. When
 * using a probe for Z Home, positions close to the edge may fail with position
 * unreachable due to probe/nozzle offset.  This can be used to avoid a model.
 *
...
/**
 * Set the laser power to false to stop the planner from processing the current
 * power setting.
 */
#if ENABLED(LASER_FEATURE)
...
// Disable the leveling matrix before homing
    ...
    const bool leveling_restore_state = parser.boolval(
        'L', TERN1(RESTORE_LEVELING_AFTER_G28, planner.leveling_active));
#endif
...
#endif
// PARKING_EXTRUDER homing requires different handling of movement / solenoid
// activation, depending on the side of homing
#if ENABLED(PARKING_EXTRUDER)
    const bool homed_towards_tool = old_tool_index == TERN(X_HOME_TO_MIN, 0, 1),
               pe_final_change_must_unpark =
                   parking_extruder_unpark_after_homing(old_tool_index,
                                                        homed_towards_tool);
#endif
    tool_change(0, true);
#endif
...
#define _UNSAFE(A) TERN0(Z_SAFE_HOMING, homeZZ&& axis_should_home(_AXIS(A)))
    ...
    const bool
        homeZZ = TERN0(HAS_Z_AXIS, parser.seen_test('Z')),
        NUM_AXIS_LIST_(  // Other axes should be homed before Z safe-homing
            ...
            NUM_AXIS_LIST_(  // Home each axis if needed or flagged
                ...
                homeY = needY || parser.seen_test('Y'), homeZ = homeZZ,
                ...
                homeW = needW || parser.seen_test(AXIS9_NAME)) home_all =
                NUM_AXIS_GANG_(  // Home-all if all or none are flagged
                    homeX == homeX, &&homeY == homeX, &&homeZ == homeX,
                    &&homeI == homeX, &&homeJ == homeX, &&homeK == homeX,
                    &&homeU == homeX, &&homeV == homeX, &&homeW == homeX)
                    NUM_AXIS_LIST(
                        doX = home_all || homeX, doY = home_all || homeY,
                        doZ = home_all || homeZ, doI = home_all || homeI,
                        doJ = home_all || homeJ, doK = home_all || homeK,
                        doU = home_all || homeU, doV = home_all || homeV,
                        doW = home_all || homeW);
    ...
    if (TERN0(HOME_Z_FIRST, doZ))
        homeaxis(Z_AXIS);

    // 'R' to specify a specific raise. 'R0' indicates no raise, e.g., for
    // recovery.resume When 'R0' is used, there should already be adequate
    // clearance, e.g., from homing Z to max.
    const bool seenR = parser.seenval('R');

    // Use raise given by 'R' or Z_CLEARANCE_FOR_HOMING (above the probe trigger
    // point)
    float z_homing_height =
        seenR ? parser.value_linear_units() : Z_CLEARANCE_FOR_HOMING;

    // Check for any lateral motion that might require clearance
    const bool may_skate =
        seenR && NUM_AXIS_ANY(doX, doY, TERN0(Z_SAFE_HOMING, doZ), doI, doJ,
                              doK, doU, doV, doW);
    ...
    if (seenR && z_homing_height == 0) {
        ...
        // The "height" for Z is a coordinate. But if Z is not trusted/homed
        // make it relative.
    ...
    // Init BLTouch ahead of any lateral motion, even if not homing with the
    // probe
...
#if HAS_X_AXIS
    if (doX ||
        (doY && ENABLED(CODEPENDENT_XY_HOMING) && DISABLED(HOME_Y_BEFORE_X))) {
...
// Home Z last if homing towards the bed
    ...
    if (doZ) {
        ...
        // finalRaiseZ = true; // DISABLED ON TT
#endif
    }
#endif

    SECONDARY_AXIS_CODE(if (doI) homeaxis(I_AXIS), if (doJ) homeaxis(J_AXIS),
                        if (doK) homeaxis(K_AXIS), if (doU) homeaxis(U_AXIS),
                        if (doV) homeaxis(V_AXIS), if (doW) homeaxis(W_AXIS));
 ...
 * This is important because it lets a user use the LCD Panel to set an IDEX
 * Duplication mode, and then print a standard G-Code file that contains a
 * single print that does a G28 and has no other IDEX specific commands in it.
 */
#if ENABLED(DUAL_X_CARRIAGE)
    ...
    TERN_(DELTA_HOME_TO_SAFE_ZONE,
          do_blocking_move_to_z(delta_clip_start_height));

#if HAS_Z_AXIS
    // Move to the configured Z only if Z was homed to MIN, because machines
    // that home to MAX historically expect 'G28 Z' to be safe to use at the end
    // of a print, and do_move_after_z_homing is not very nuanced.
    if (finalRaiseZ)
        do_move_after_z_homing();
#endif

    TERN_(CAN_SET_LEVELING_AFTER_G28,
          if (leveling_restore_state) set_bed_leveling_enabled());

// Restore the active tool after homing
#if HAS_MULTI_HOTEND && (DISABLED(DELTA) || ENABLED(DELTA_HOME_TO_SAFE_ZONE))
    tool_change(old_tool_index,
                TERN(PARKING_EXTRUDER, !pe_final_change_must_unpark,
                     DISABLED(DUAL_X_CARRIAGE)));  // Do move if one of these
#endif
