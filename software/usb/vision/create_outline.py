...
def create_outline(img):
    ...
    reference_hue = 100 # green = 120 degrees, yellow = 60 degrees in hsv
    ...
    for x in range(img.width):
        for y in range(img.height):
            ...
            h = h * 360 - reference_hue # center hue values to green
            h = np.deg2rad(h)

            if(lowest_hue < h < highest_hue):
                excess_green = np.cos(h) # cos allows looping hue range with 'greeness' between -1 and 1
            else:
                excess_green = 0 # filter weak greens
            if(s < lowest_sat or v < lowest_val):
                excess_green = 0 # filter weak saturation (background)
    ...
    path = paths[0] # each entry contains paths for each level (only one used)

    # Convert path data to numpy arrays
    points = []
    ops = []
    ...
    # Determine longest path (SHOULD result in plant 'outline')
    length = 0
    max_length = 0
    start_index = 0
    i = 0
    for vertices, code in path.iter_segments():
        ...
        if code == 1: # MOVETO = np.uint(1) (denotes new contour line)
            index = i

        if code == 79 or code == 1: # CLOSEPOLY = np.uint8(79) (denotes end of contour line)
            length = 0
    ...
    # Extract longest contour line and zip with corresponding codes
    ...
    # Create black & white image of plant silhouette
    for x in range(img.width):
        for y in range(img.height):
